...
export default function Shop() {
  ...
  const handleApplyPricee = (min, max) => {
    setMinPrice(min);
    setMaxPrice(max);
  };
  ...
  const setFilteredData = () => {
    let filteredData = products.filter(product => {
      ...
      //const matchesPrice =
      //  (minPrice ? product.base_price >= minPrice : true) &&
      //  (maxPrice ? product.base_price  <= maxPrice : true);
    //  const matchesPrice = 
    //    (minPrice !== '' && product.base_price >= minPrice || minPrice === '') && 
    //    (maxPrice !== '' && product.base_price <= maxPrice || maxPrice === '');
...
function ProductCard({ product }) {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/rates/${product.id}`);
        const data = response.data;

        // Ensure avg_stars defaults to 0 if it's undefined or null
  ...
  <div style={{ fontSize: '14px', color: '#ab0965', fontWeight: 'bold' }}>
    {/* Check if discount exists and calculate current price */}
    {product.discount > 0 ? (
      <>
      ...
      // If no discount, just show base price
      <>Tk {product.base_price}</>
    )}
  </div>
  <div style={{ fontSize: '12px', marginTop: '0px' }}>
        {ratingData ? (
          <>
            <div>{ratingData.avg_stars} / 5
              <label style={{color:'#d8a444', fontSize:'16px', padding:'0px 2px'}}>★</label>
               ({ratingData.total})</div>
          </>
        ) : (
